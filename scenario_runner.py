...
from srunner.scenarios.route_scenario import (RouteScenario, convert_transform_to_location)
...
class ScenarioRunner(object):
    ...
    def _load_and_run_scenario(self, config):
        ...
        if self._args.agent:
            ...
            try:
                print("!!!!! if self._args.agent")
                print(self.module_agent)
                print(agent_class_name)
                print(self._args.agentConfig)
                self.agent_instance = getattr(self.module_agent, agent_class_name)(self._args.agentConfig)
                config.agent = self.agent_instance

                config.trajectory = [
                    carla.Location(x=116, y=-3.5, z=0),
                    carla.Location(x=103, y=-3.5, z=0),
                    carla.Location(x=90, y=-3.5, z=0)
                ]
                ...
                config.scenario_file = "C:\\Users\\taha\\Desktop\\42scenariosModified.json"

                world_annotations = RouteParser.parse_annotations_file(config.scenario_file)
                potential_scenarios_definitions, _ = RouteParser.scan_route_for_scenarios(config.town, route, world_annotations)
                sampled_scenarios_definitions = self._scenario_sampling(potential_scenarios_definitions)
        ...
        try:
            ...
            if self._args.openscenario:
                print("!!!!!!")
                print("inside if self._args.openscenario")
                ...
                scenario.list_scenarios = self._build_scenario_instances(
                    world=self.world,
                    ego_vehicle=self.ego_vehicles[0],
                    scenario_definitions=sampled_scenarios_definitions
                )

                print(scenario.list_scenarios)
            elif self._args.route:
                print("!!!!!!")
                print("inside elif self._args.route")
